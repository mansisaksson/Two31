...
void SSubstanceEditorPanel::OnBrowseImageInput(UObject* ImageInput)
{
	if (ImageInput)
	{
		TArray< UObject* > Objects;
		Objects.Add(ImageInput);
		GEditor->SyncBrowserToObjects(Objects);
	}
}
		...
		[
			SNew(SHorizontalBox)
				...
				+ SHorizontalBox::Slot()
					...
					.Padding(2.0f, 1.0f)
					[
						PropertyCustomizationHelpers::MakeBrowseButton(FSimpleDelegate::CreateSP(this, &SSubstanceEditorPanel::OnBrowseImageInput, ImageInput->ImageSource))
					]
				+SHorizontalBox::Slot()
					.AutoWidth()
					.HAlign(HAlign_Left)
