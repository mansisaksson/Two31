	...
	for (int32 IdxMip=0 ; IdxMip < ResultText.mipmapCount ; ++IdxMip)
	{
		...
		if (Texture->Format == PF_B8G8R8A8)
		{
			...
			for (SIZE_T Idx = 0; Idx < ImageSize; Idx +=4)
			{
				...
				// r, g, b, a = b, g, r, a
				DstPixels[Idx+0] = SrcPixels[Idx+2];
				DstPixels[Idx+1] = SrcPixels[Idx+1];
				DstPixels[Idx+2] = SrcPixels[Idx+0];
				DstPixels[Idx+3] = SrcPixels[Idx+2];
			}
		}
		else
		{
...
void TearDownSubstance()
{
	...
	AsyncQueue.Empty();
	CurrentRenderQueue.Empty();
	BlueprintQueue.Empty();
	
	DelayedImageInputs.Empty();
	ActiveImageInputs.Empty();
	...
	USubstanceGraphInstance* SourceGraphInstance)
{
	...
	CopyInstance(SourceGraphInstance->Instance, NewInstance);
