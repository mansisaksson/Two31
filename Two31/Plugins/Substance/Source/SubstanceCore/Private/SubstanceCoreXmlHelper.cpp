...
DEFINE_LOG_CATEGORY_STATIC(LogSubstanceEditorXml, Log, All);
...
bool ParseOutput(const XMLNode* OutputNode, graph_desc_t* ParentGraph)
{
	...
	const XMLNode* OutputGuiNode = OutputNode->FindChildNode("outputgui");
	...
	if (OutputGuiNode)
	{
		...
		const XMLNode* ChannelsNode = OutputGuiNode->FindChildNode("channels");

		if (ChannelsNode)
		{
			const XMLNode* ChannelNode = ChannelsNode->FindChildNode("channel");
			...
			if (ChannelNode)
			{
				...
				else if (FString(TEXT("normal")) == Attribute)
				{
					...
					if (Output.Format != Substance_PF_DXT5)
					{
						...
						{
							UE_LOG(LogSubstanceEditorXml, Warning, TEXT("The current settings requires normal maps to be DXT5 with a *X*Y layout."));
						}
					}
				}
...
bool ParseInput(const XMLNode* InputNode, graph_desc_t* ParentGraph)
{
	...
	const XMLNode* InputguiNode = InputNode->FindChildNode("inputgui");
	if (!InputguiNode)
	{
	...
	else
	{
		...
		else if (Widget == FString(TEXT("combobox")))
		{
			...
			const XMLNode* GuicomboboxNode = 
				InputguiNode->FindChildNode("guicombobox");
			
			if (GuicomboboxNode)
			{
				const XMLNode* GuicomboboxitemNode = 
		...
		const XMLNode* InputsubguiNode = InputguiNode->GetFirstChildNode();
		if (InputsubguiNode)
		{
	...
	TArray<uint32>& assembly_uid)
{
	...
	{
		// abort if invalid
		UE_LOG(LogSubstanceEditorXml, Error, TEXT("Cannot import, sbsar does not contain xml graph description."));
	...
	for (;ItXml;++ItXml)
	{
		XMLFile Document(TCHAR_TO_UTF8(*(*ItXml)));

		if (!Document.IsValid())
		{
			UE_LOG(LogSubstanceEditorXml, Error, TEXT("Cannot import, invalid xml graph description."));
			return false;
		}
		...
		if (!RootNode)
		{
			UE_LOG(LogSubstanceEditorXml, Error, TEXT("Cannot import, invalid xml graph description (sbsdescription)."));
			return false;
		}
		else
		{
			...
			if (Attribute.Len() <= 0)
			{
				UE_LOG(LogSubstanceEditorXml, Error, TEXT("Cannot import, invalid xml graph description (asmuid)."));
				return false;
			}
			assembly_uid.Add(appAtoul(*Attribute));

			const XMLNode* GlobalNode = RootNode->FindChildNode("global");

			const XMLNode* normalformatinputnode = NULL;
			const XMLNode* timeinputnode = NULL;

			if (GlobalNode!=NULL)
			{
				const XMLNode* inputlistnode = 
					GlobalNode->FindChildNode("inputs");

				const XMLNode* inputnode = 
			...
			const XMLNode* GraphListNode = RootNode->FindChildNode("graphs");
			if (!GraphListNode)
			{
				UE_LOG(LogSubstanceEditorXml, Error, TEXT("Cannot import, invalid xml graph description (graphs)."));
				return false;
			}
			...
			if (0 == GraphCount)
			{
				UE_LOG(LogSubstanceEditorXml, Error, TEXT("Cannot import, invalid xml graph description (no graph)."));
				return false;
			}

			// Parse all the graphs
			for (const XMLNode* GraphNode = GraphListNode->GetFirstChildNode(); GraphNode != NULL;GraphNode = GraphNode->GetNextNode())
			{
				...
				// Parse outputs
				{
					const XMLNode* OutputListNode = GraphNode->FindChildNode("outputs");
		
					if (!OutputListNode)
					{
						UE_LOG(LogSubstanceEditorXml, Error, TEXT("Cannot import, invalid xml graph description (no outputs for %s)"), *OneGraph->Label);
						return false;
					}
					...
					for (const XMLNode* OutputNode = OutputListNode->GetFirstChildNode(); OutputNode != NULL;OutputNode = OutputNode->GetNextNode())
					{
						if (!ParseOutput(OutputNode,OneGraph.get()))
						{
							UE_LOG(LogSubstanceEditorXml, Error, TEXT("Cannot import, invalid xml graph description (invalid output)"));
							return false;
						}
					}
				}
				...
				// Sort outputs by uid
				OneGraph->commitOutputs();

				// Parse inputs
				{
					const XMLNode* InputListNode = GraphNode->FindChildNode("inputs");

					if (!InputListNode)
					{
						UE_LOG(LogSubstanceEditorXml, Error, TEXT("Cannot import, invalid xml graph description (no inputs)"));
						return false;
					}
					...
					if	(InputCount >= 0)
					{
						...
						for (const XMLNode* InputNode = InputListNode->GetFirstChildNode(); InputNode != NULL;InputNode = InputNode->GetNextNode())
						{
							// Parse input
							if ( !ParseInput(InputNode, OneGraph.get()))
							{
								UE_LOG(LogSubstanceEditorXml, Error, TEXT("Cannot import, invalid xml graph description (invalid input)"));
								return false;
							}
						}
...
{
#if WITH_EDITOR
	XMLFile XmlDocument(TCHAR_TO_UTF8(*XmlContent));

	if (false == XmlDocument.IsValid())
	{
		UE_LOG(LogSubstanceEditorXml, Error, TEXT("Invalid preset file (xml parsing error)."));
		return false;
	}

	const XMLNode* RootNode = XmlDocument.GetRootNode();
	const XMLNode* sbspresetnode = RootNode->FindChildNode("sbspreset");
	...
	for (const XMLNode* child = RootNode->GetFirstChildNode(); child != NULL;child = child->GetNextNode())
	{
		...
		for (const XMLNode* presetinputnode = child->GetFirstChildNode(); presetinputnode != NULL;presetinputnode = presetinputnode->GetNextNode())
		{
