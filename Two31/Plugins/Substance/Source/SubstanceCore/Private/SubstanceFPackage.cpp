	...
	const FString& FilePath)
{
...

	{
		...
		Err = SubstanceLinkerContextInit(&context, mode);
		...
		Err = SubstanceLinkerContextSetXMLCallback(context, (void*)linkerCallbackArchiveXml);
		if (Err)
		{
		...
		SubstanceLinkerHandle *handle;
		Err = SubstanceLinkerHandleInit(&handle,context, 0);
		if (Err)
		{
			GWarn->Log((*FString::Printf(TEXT("Substance: linker error (%d)"), Err)));
			return;
		...
		// Release
		Err = SubstanceLinkerRelease(handle, context);
		if (Err)
		{
			GWarn->Log((*FString::Printf(TEXT("Substance: linker error (%d)"), Err)));
			return;
	...
	for (auto itGr = Graphs.itfront(); itGr; ++itGr)
	{
		if ((*itGr)->LoadedInstances.Num() == (*itGr)->InstanceUids.Num())
		{
			for (auto itInst = (*itGr)->LoadedInstances.itfront(); itInst; ++itInst)
			{
				...
				{
					//early exit if any graph needs link data
					return;
				}
			}
		}
