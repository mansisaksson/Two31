...
bool Substance::Details::LinkDataAssembly::push(LinkContext& cxt) const
{
	...
	// Push assembly
	{
		err = SubstanceLinkerPushAssembly(cxt.handle, cxt.stateUid, mAssembly.data(), mAssembly.size());
	}
	...
	SBS_VECTOR_FOREACH (const OutputFormat& outfmt, mOutputFormats)
	{
		...
		if (trres.second)
		{
			err = SubstanceLinkerSetOutputFormat(
				cxt.handle,
				trres.first,
				outfmt.format,
				outfmt.mipmap);
...
bool Substance::Details::LinkDataStacking::push(LinkContext& cxt) const
{
	...
	{
		int32 err = SubstanceLinkerConnectOutputToInput(
			cxt.handle,
