	...
	unsigned int newUid)
{
	...
	res = SubstanceLinkerGetUserData(handle,(size_t*)&engine);
	check(res==0);
	...
	size_t jobUserData)
{
	...
	unsigned int res = SubstanceHandleGetTexture(handle, outputIndex, &texture, &context);
	check(res == 0);
...
void Substance::Details::Engine::createLinker()
{
	...
	res = SubstanceLinkerContextInit(&mLinkerContext, Substance_Linker_Context_Init_Auto);
	check(res==0);	
	
	// Set UID collision callback
	res = SubstanceLinkerContextSetUIDCollisionCallback(mLinkerContext, (void*)substanceDetailsLinkerCallbackUIDCollision);
	check(res==0);	
	
	// Create linker handle
	res = SubstanceLinkerHandleInit(&mLinkerHandle, mLinkerContext, (size_t)this);
	assert(res==0);
}
...
{
	unsigned int res;
	res = SubstanceLinkerRelease(mLinkerHandle, mLinkerContext);
	mLinkerCacheData = NULL;
	check(res==0);
}
...
bool Substance::Details::Engine::link(RenderJob* renderJobBegin)
{
	uint32 res;
	(void)res;
	...
	res = SubstanceLinkerEnableOutputs(
		mLinkerHandle,
		enabledIds.GetData(),
		enabledIds.Num());
	...
	res = SubstanceLinkerGetCacheMapping(
		mLinkerHandle,
		&mLinkerCacheData,
		mLinkerCacheData);
	check(res==0);
	...
	res = SubstanceHandleInit(&newhandle,
	...
	const RenderOptions& renderOptions)
{
	...
	{
		// Switch resources
		res = SubstanceHandleSwitchHard(mHandle, Substance_Sync_Asynchronous, &mHardResources);
		check(res==0);
	
		// Get current engine state
		res = SubstanceHandleGetState(
			mHandle,
			&state);
...
void Substance::Details::Engine::clearCache()
{
	...
	if (mHandle != NULL)
	{
		...
		SubstanceHandleSwitchHard(mHandle, Substance_Sync_Synchronous, &substanceRscnNoRes);
		SubstanceHandleStart(mHandle);
		...
		SubstanceHandleSwitchHard(mHandle, Substance_Sync_Synchronous, &mHardResources);
		SubstanceHandleStart(mHandle);
	}
}
	...
	mContext(NULL)
{
	...
	// Create context
	res = SubstanceContextInit(&mContext,
