...
Substance::Details::RendererImpl::processJob(RenderJob *begjob)
{
	...
	RenderJob* lastjob = begjob;
	bool pullidentity = !mCancelOccur;
	...
	
	{
		...
		do
		{
			...
			pullidentity = pullidentity && 
				lastjob->getState()==RenderJob::State_Computing;
	...
	if (mEngine.getHandle()!=NULL)
	{
		...
		Computation computation(mEngine,!pullidentity);
		
		// If pull is necessary (not strict resume)
		if (!pullidentity)
		{
			// Push all I/O to engine
			RenderJob* curjob = begjob;
			do
			{
				// Push I/O
